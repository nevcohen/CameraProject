...
public class Render {
	...
	private boolean multyRays = false;
	...
	private void castRay(int nX, int nY, int col, int row) {
		...
		List<Ray> allRays = camera.constructRaysThroughPixel(nX, nY, col, row);
		int size = allRays.size();
		if (size == 1) {
			imageWriter.writePixel(col, row, tracer.traceRay(allRays.get(0)));
			return;
		}
		for (Ray ray : allRays)
			color = color.add(tracer.traceRay(ray));

		imageWriter.writePixel(col, row, color.reduce(size));
	}
